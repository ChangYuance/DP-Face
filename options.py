...
class Options(object):
    ...
    def initialize(self):
        ...
        # basic settings
        parser.add_argument('--root', type=str, default='/home/chang_yuance/data/changyuance/codes/TFace-master/attribute/M3DFEL', help='Root directory for datasets')
        parser.add_argument('--mode', type=str, default="train")
        parser.add_argument('--val', type=bool, default=True, help='Whether to use validation (default: True)')
        parser.add_argument('--audio', type=bool, default=False, help='Whether to use validation (default: True)')
        parser.add_argument('--center_loss', type=bool, default=False, help='Whether to use center loss (default: False)')
        parser.add_argument('--focal_loss', type=bool, default=False, help='Whether to use focal loss (default: False)')
        parser.add_argument('--AUs', type=bool, default=False, help='Whether to use validation (default: False)')
        parser.add_argument('--fusionmodel', type=str, default="None")
    ...
    def parse(self):
        ...
        if args.dataset == "DFEW_val":
            args.train_dataset = os.path.join(
                args.root, "EmoLabel_DataSplit/train(single-labeled)/set_X.csv")
            args.test_dataset = os.path.join(
                args.root, "EmoLabel_DataSplit/test(single-labeled)/set_X.csv")
            args.val_dataset = os.path.join(
                args.root, "EmoLabel_DataSplit/test(single-labeled)/set_X.csv")
        # set the fold
