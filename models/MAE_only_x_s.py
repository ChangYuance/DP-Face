...
class MAE_only_x_s(nn.Module):
    ...
    def __init__(self, args):
        ...
        #         nn.init.zeros_(m.bias)
        # self.mae.apply(init_weights)
        # self.mae.eval()
        for p in self.mae.parameters():
            p.requires_grad = False
        for blk in self.mae.encoder.blocks[8:]:
            for p in blk.parameters():
                p.requires_grad = True
        # 3. 解冻 encoder 的 LayerNorm（非常关键）
        for p in self.mae.encoder.norm.parameters():
            p.requires_grad = True
    ...
    def forward(self, x, phrase='train'):
        ...
        feat_seq = self.mae.extract_features(x, keep_seq=True)  # (B, k, 768)
        feat = feat_seq.mean(dim=1)  # 比 mean 强
        pred_logit = self.decoder(feat)  # (B, d_model) -> (B, d_output)
        return pred_logit
