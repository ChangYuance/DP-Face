...
from utils import *
class MAE_x_x_a_s_center_loss(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.center_loss = CenterLoss(num_classes=2, feat_dim=384+527, device=self.device, lambda_center=0.5)
    def forward(self, x, x_a, phrase='train'):
        ...
        if x_a.shape[1] ==1:
            x_a = x_a.squeeze()
        x_a = self.ast(x_a) # (B, 527)
        fea = torch.cat([x,x_a],axis=-1)
        # [batch, bag_size, 1024]
        # ---- 映射 & 归一化 ----
